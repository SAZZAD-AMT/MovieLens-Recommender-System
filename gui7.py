...
def update_comboboxes(event=None):
    ...
    if selected_movie:
        selected_movie_genres = movies[movies['title'] == selected_movie]['genres'].iloc[0]
        unique_genres = set(selected_movie_genres.split('|'))
        genre_combobox.config(values=list(unique_genres))

movie_combobox = ttk.Combobox(window, width=50)
movie_combobox.pack()
movie_combobox.bind("<KeyRelease>", update_comboboxes)

recommend_button = ttk.Button(window, text='GET GENRES', command=update_comboboxes)
recommend_button.pack(pady=10)

genre_combobox = ttk.Combobox(window, width=50)
genre_combobox.pack()
genre_combobox.bind("<KeyRelease>", recommend_movies)
